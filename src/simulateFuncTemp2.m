...
AGORAModelWithExpr = [];
disp(['i ' num2str(i)])
if realOneSpecies || ~doSimulation
    if ~doSimulation
	if ~isempty(AGORAMat{i})
		...
		if useFBA==0
		    writeData({AGORAModel.rxnECNumbers},[inputDir filesep 'MGMData/' ucrFolder '/speciesSep' num2str(i) '.modelec'],'\t');
                    if justTwoSpecies
                        system(['python /mnt/vdb/home/ubuntu2/MATLAB/SEED/src/simulateSmallModelsSeparatePicrustAGORA.py False ' ucrFolder ' ' AGORAModel.description ' ' num2str(i) ' True']);
		    else
		        system(['python /mnt/vdb/home/ubuntu2/MATLAB/SEED/src/simulateSmallModelsSeparatePicrustAGORA.py False ' ucrFolder ' ' AGORAModel.description ' ' num2str(i) ' False']);
                    end
	...
	if i~=j && ~isempty(AGORAMat{i}) && ~isempty(AGORAMat{j})
		...
		if mergeModelsAGORAFlag
			...
			writeData({AGORAModel2.rxnECNumbers},[inputDir filesep 'MGMData/' ucrFolder filesep AGORAModel1.description AGORAModel2.description 'species2.modelec'],'\t');
                        if IBDFlag
			    system(['python /mnt/vdb/home/ubuntu2/MATLAB/SEED/src/simulateSmallModelsSeparatePicrustAGORA.py True ' ucrFolder ' ' AGORAModel1.description ' ' AGORAModel2.description ' False True']);
                        else
			    system(['python /mnt/vdb/home/ubuntu2/MATLAB/SEED/src/simulateSmallModelsSeparatePicrustAGORA.py True ' ucrFolder ' ' AGORAModel1.description ' ' AGORAModel2.description ' False False']);
                        end
			...
			if useFBA==0
				...
				expressionData = species1expr;
			        ...
			        species1file = [inputDir filesep 'MGMData/' ucrFolder filesep 'speciesSep' num2str(i) '.modelexpr'];
disp(species1file)
disp(exist('species1file','file'))
                                if exist(species1file,'file')
			      
			            species1expr = importdata(species1file);
				else
				    return
				end
				species2file = [inputDir filesep 'MGMData/' ucrFolder filesep 'speciesSep' num2str(j) '.modelexpr'];
                                if exist(species2file,'file')
                                    species2expr = importdata(species2file);
                                else
                                    return
                                end
				expressionData = species1expr;			      
