...
fluxesFBA = {};
fbafits = [];
ucrFolderMH0007Mistake = 'ycrC97MH0007';
mergeModelsAGORA = 0;
for i=1:length(AGORAMat)
	...
	if i~=j && ~isempty(AGORAMat{i}) && ~isempty(AGORAMat{j})
            if mergeModelsAGORA
		fileFBA = [outputDir1 filesep 'FBASims' filesep num2str(i) '_' num2str(j) '_3_' ucrFolderMH0007 '_FBA.flux'];
		if exist(fileFBA,'file')
		    disp('THERE')
		    AGORAFBAFluxes = importdata(fileFBA);
		    load(AGORAMat{i})
		    AGORAModel1 = AGORAModel;
		    load(AGORAMat{j})
		    AGORAModel2 = AGORAModel;
		    AGORAModel3 = mergeModelsAGORA(AGORAModel1,AGORAModel2);
		    AGORAModel3 = assignSortedBiom(AGORAModel3);
		    tempA = regexp(cellfun(@(x) x{1}, AGORAModel3.subSystems, 'UniformOutput',0),'xchange');
		    fbafits(i,j) = sum(abs(AGORAFBAFluxes(cellfun(@(x) ~isempty(x), tempA))));
		end
	    else
	        fileFBA1 = [outputDir1 filesep 'FBASimsYCR' filesep num2str(i) '_' num2str(j) '_1_' ucrFolderMH0007Mistake '_FBA.flux'];
	        fileFBA2 = [outputDir1 filesep 'FBASimsYCR' filesep num2str(i) '_' num2str(j) '_2_' ucrFolderMH0007Mistake '_FBA.flux'];
                if exist(fileFBA1,'file') && exist(fileFBA2,'file')
		disp('HERE')
                    load(AGORAMat{i})
		    fbaFluxes = importdata(fileFBA1);
		    modelsFBA{end+1} = AGORAModel;
		    fluxesFBA{end+1} = fbaFluxes;
		    load(AGORAMat{j})
		    fbaFluxes = importdata(fileFBA2);
		    modelsFBA{end+1} = AGORAModel;
		    fluxesFBA{end+1} = fbaFluxes;
                    if length(modelsFBA) > 500
                        nonsense = nonsense+1;
                    end
                end
	    end
...
for z=1:2
	...
	modelsPick = modelsFBA;
        fluxesPick = fluxesFBA;	  
    end
	...
	for j=1:length(modelsPick)
	    ...
	    ithTotalTerm = ithTotalTerm - compTerm + coopTerm;
	        ...
	        matrixBasic1(i,j) = -compBasicTerm + coopBasicTerm;
	    elseif z==2
	        matrixMH0007(i,j) = -compTerm + coopTerm;
	        matrixBasic2(i,j) = -compBasicTerm + coopBasicTerm;
	    else
	        matrixFBA(i,j) = -compTerm + coopTerm;
	    end
	end
	if z==1
	...
	elseif z==2
	    ...
	    ithTotalTermArrFBA(i) = ithTotalTerm;	  
        end
    end
end
