...
iMH551ECs = cellfun(@(x) strsplit(x,{', ','/ ','/',' and ',' or ',',',';'}), iMH551ECs, 'UniformOutput',0); iMH551ECs = [iMH551ECs{:}]; iMH551ECs = unique(iMH551ECs);
...
iBif452ECs = cellfun(@(x) strsplit(x,{';',':'}), iBif452ECs, 'UniformOutput',0);
iBif452ECs = [iBif452ECs{:}]; iBif452ECs = unique(iBif452ECs);
iBif452ECs = iBif452ECs(cellfun(@(x) isempty(regexp(x,'[KA]')), iBif452ECs));
